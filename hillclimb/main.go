...
func evaluateFitness(config *dense.NetworkConfig, mnist *dense.MNISTData) float64 {
	...
	for i, image := range mnist.Images {
		...
		// Assuming the network outputs probabilities for digits 0-9
		predictedDigit := 0
		highestProb := 0.0
		for k := 0; k < 10; k++ {
...
func main() {
	...
	if !fileExists("best_model.json") {
		...
		initialConfig := createFFNNModel(numInputs, numOutputs, outputActivationTypes)
...
// Function to create a simple FFNN model for MNIST
func createFFNNModel(numInputs, numOutputs int, outputActivationTypes []string) *dense.NetworkConfig {
	modelID := "ffnn_model"
	projectName := "FFNN MNIST"
	...
	// Define hidden layers (you can adjust the number of neurons)
